...
const perAar: Record<number, { kap: number[]; kunder: number[]; andel: number[]; staff: number[]; total: number[]; launches: number[]; dl1: boolean[] }> = {};
...
for (let seed = 1; seed <= N; seed++) {
  ...
  while (s.uge < slutUge && !s.slut) {
    ...
    if (s.uge % 52 === 0) {
      ...
      const pa = (perAar[a] ??= { kap: [], kunder: [], andel: [], staff: [], total: [], launches: [], dl1: [] });
...
console.log('år    kapital  kunder  dk-andel% staff bedste40 lanceringer DL#1%');
for (const [a, p] of Object.entries(perAar)) console.log(a, med(p.kap).toFixed(1).padStart(8), String(Math.round(med(p.kunder))).padStart(7), med(p.andel).toFixed(1).padStart(8), String(med(p.staff)).padStart(5), String(med(p.total)).padStart(8), String(med(p.launches)).padStart(10), String(pct(p.dl1)).padStart(6));
