...
import type { AcqChannel, Action, AgentFunktion, GameState, MarketId, PlatformKind, ProductTypeId, Role, ThemeId, Vertical } from '../../src/sim/types';
...
import { agentStatus, transformationStatus } from '../../src/sim/agents';
...
export const BALANCERET: Profil = {
  ...
  kanaler: { soeg: 0.3, affiliate: 0.3, sociale: 0.25, crm: 0.15 },
  runder: true, markeder: 4, offshore: false, platform: true, ai: 'ansvarlig', overvaagning: 0.8, hyper: false, transformation: 0.25,
  ansvarsforskning: true, tillidsstyring: true, andenVertikal: true, sponsorater: true,
};

export const GRAADIG: Profil = {
  navn: 'Grådig', intensitet: 5, bonus: 3, vip: 3, marketingAndel: 0.35,
  kanaler: { tv: 0.25, streamere: 0.2, affiliate: 0.3, soeg: 0.15, crm: 0.1 },
  runder: true, markeder: 7, offshore: true, platform: false, ai: 'hensynsloes', overvaagning: 0.2, hyper: true, transformation: 0.5,
  ansvarsforskning: false, tillidsstyring: false, andenVertikal: true, sponsorater: true,
};

export const FORSIGTIG: Profil = {
  navn: 'Forsigtig', intensitet: 2, bonus: 0, vip: 0, marketingAndel: 0.15,
  kanaler: { soeg: 0.45, affiliate: 0.3, crm: 0.25 },
  runder: false, markeder: 1, offshore: false, platform: false, ai: 'ansvarlig', overvaagning: 0.9, hyper: false, transformation: 0,
  ansvarsforskning: true, tillidsstyring: true, andenVertikal: true, sponsorater: false,
};

export const AI_AFVISER: Profil = { ...BALANCERET, navn: 'AI-afviser', ai: 'ingen', transformation: 0 };

export const AI_HENSYNSLOES: Profil = { ...BALANCERET, navn: 'AI-hensynsløs', ai: 'hensynsloes', overvaagning: 0.2, hyper: true, transformation: 0.5 };
...
function aiBeslutninger(p: Profil, s: GameState, buffer: number): Action[] {
  ...
  for (const f of AGENT_ORDEN) {
    ...
    if (f === 'risiko' && p.ai === 'hensynsloes') continue;
...
export function lavBot(p: Profil): Bot {
  return {
    navn: p.navn,
    beslut(s) {
      const a: Action[] = [];
      ...
      if ((s.projekter.length === 0 && s.uge < 3) || (s.kapital < 0.6 && heltLedige.length > 0)) {
      ...
      if (s.staff.length < loft && s.kapital > buffer + 0.5) {
      ...
      const bonus = (presset ? 0 : p.bonus) as 0 | 1 | 2 | 3;
      const vip = (presset ? 0 : p.vip) as 0 | 1 | 2 | 3;
      ...
      // Marketing
      if (s.produkter.some((x) => x.ejer === 'spiller' && x.aktiv) && s.uge % 4 === 0) {
        const budget = Math.max(0.01, s.regnskab.bsi * p.marketingAndel);
      ...
      // Offshore-fristelsen
      if (p.offshore && !s.offshoreBrand && aar >= 2014 && s.produkter.some((x) => x.ejer === 'spiller') && s.kapital > 4) a.push({ t: 'setOffshoreBrand', aktiv: true });
      ...
      if (p.platform) {
        ...
        for (const kind of kinds) {
          ...
          const maal = pl.model === 'hybrid' ? (aar >= 2018 ? 'egen' : null) : pl.model === 'egen' ? null : 'hybrid';
          if (!maal) continue;
      ...
      // Opkøbstilbud afvises; sponsorater bydes på i egne markeder
