...
import { computeSchedule, type Milestone, type MilestoneId, type PlanInput } from '../../lib/schedule';
...
export function ResultView({
  ...
  const milestones = useMemo(() => computeSchedule(input), [input]);
  ...
  const handleCopy = async () => {
    const ok = await copyText(renderPlanText(milestones, input));
    showToast(ok ? 'Planen er kopieret' : 'Kunne ikke kopiere planen');
  };

  const handleShare = async () => {
    const res = await sharePlan('Min bageplan for surdejsbrød', renderPlanText(milestones, input));
