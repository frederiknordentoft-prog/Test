...
check('deep-link: gears braked immediately', (await motion()) === 'stopped' && (await cssRunning()) === 0, `${await motion()} css=${await cssRunning()}`)
...
check('true reload: gears braked at once', (await motion()) === 'stopped' && (await cssRunning()) === 0, `${await motion()} css=${await cssRunning()}`)
...
await page.waitForTimeout(60)
const justAfter = await angles()
check('bottleneck: brake starts from the current angle (no jump)', maxStep(before, justAfter) < 10, `max step ${maxStep(before, justAfter).toFixed(1)}°`)
await page.waitForTimeout(240)
check('bottleneck: decelerating at 300ms (compositor animation, CSS spin off)', (await motion()) === 'braking' && (await waapiRunning()) >= 5 && (await cssRunning()) === 0, `${await motion()} waapi=${await waapiRunning()} css=${await cssRunning()}`)
const a300 = await angles()
await page.waitForTimeout(1100)
const a1400 = await angles()
await page.waitForTimeout(200)
const a1600 = await angles()
check('bottleneck: stopped after ~1.2s and holding still', (await motion()) === 'stopped' && maxStep(a1400, a1600) < 0.01 && maxStep(a300, a1400) > 0.5, `${await motion()} drift=${maxStep(a1400, a1600).toFixed(3)}° travelled=${maxStep(a300, a1400).toFixed(1)}°`)
...
// remove → gears resume, ease-in, then hand over to the CSS animation without a jump
await page.locator('.face[data-component="mennesker"]').click({ button: 'right' })
await page.waitForTimeout(200)
...
check('core bottleneck via hash', await page.locator('.clockwork[data-bottleneck="true"]').count() === 1 && (await motion()) === 'stopped' && (await cssRunning()) === 0, `${await motion()}`)
...
check('reduced motion off mid-session while braked: gears stay stopped', (await motion(p2)) === 'stopped' && (await cssRunning(p2)) === 0, `${await motion(p2)} css=${await cssRunning(p2)}`)
await p2.keyboard.press('b')
await p2.waitForTimeout(1300)
check('reduced motion off: release → gears run', (await motion(p2)) === 'running' && (await cssRunning(p2)) >= 5, `${await motion(p2)} css=${await cssRunning(p2)}`)
