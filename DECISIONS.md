...
## PC-optimering + vinkel-ikoner + visuelt løft
- **PieceIcon: true-silhouette-at-true-angle SVG icons replace all text glyphs on slot buttons, the radial picker and the palette** — the old '／' glyph carried its own built-in ~45° slant, so every rotation read 45° wrong; a flat 0° plank now LOOKS flat, and each ring button previews exactly the orientation you get.
- **Radial ring staggers alternate angles onto two radii** — 22.5° neighbours can never overlap regardless of button size or screen; ring centre still clamps to the board.
- **`#root` is now a HARD 100dvh (was min-height) with overflow hidden** — the layout must always shrink content (board rescales, level list scrolls internally) instead of growing the document; this both fixed the desktop board never filling its column (percentage heights don't resolve against auto-height flex parents) and the post-win page overflow on mobile.
- **Desktop layout: two-column game view** (board left, controls right, vertically centred), board scale cap raised 460→700 px, level select in a two-column grid, Space = drop / Esc = close picker.
- **The art stays 100% procedural canvas — no PNG assets** — crisp at every DPI/scale, zero network, deterministic; the pass adds a machine frame + dot grid + drop-spotlight board, a riveted metal dispenser, glow-backed piece art (wood-grain look on breakables with straps, taut membrane + coils on the bouncer, hot capsule + nozzle on the booster, swirl portal), a goal ring with a gold star, hazard-teeth fail zones and a fading trail.
